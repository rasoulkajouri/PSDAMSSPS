...
bool swimmer :: distanceVicsekZone(swimmer* agent[], const int i, const int j){
	...
	if(viewField == 360) return dx*dx+dy*dy<orienR*orienR;
	...
	return dx*dx+dy*dy<orienR*orienR && abs(orientation - angle) <= viewField * 0.5;
}
